...
:: ------------------------------------------------------------------
:: Call a custom script if it exists before we get source files.
:: For example, the script can be used to cloak branches in the workspace
:: ------------------------------------------------------------------
set _customCmdFile="%_LocalFolder%\CodeBox\override\Tools\preSourceRetrieve.cmd"
if exist %_customCmdFile% (
  call %_customCmdFile%
)
...
if exist "%INETROOT%\CodeBox\CheckinApproval\UpdateCodeBoxCheckinPolicies.exe" (
  ...
  IF ERRORLEVEL 1 (
    @echo Warning: TFS check in policies must be updated as an administrator.
  )
  @echo off
)
...
:: TFS Shortcut
set "Args=/k cd /d _INETROOT_&set INETROOT=_INETROOT_&set CODEBOXPROJECT=%Project%&codebox\tools\runme-tfs.cmd"
